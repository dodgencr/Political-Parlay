...
export async function readData(){
 ...
 try{const result=await env.DB!.prepare('SELECT key, payload FROM snapshots').all<{key:string,payload:string}>();for(const row of result.results)data[row.key]=JSON.parse(row.payload);return {data,persistent:true};}catch{return {data,persistent:false};}
}
